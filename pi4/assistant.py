                ...
                while b"\n" in buf:
                    ...
                    try:
                        ...
                        if hdr.get("type") == "detection":
                            ...
                            print(f"[OWW]  score={score:.3f} (threshold={OWW_THRESHOLD})", flush=True)
...
def _playback_interrupt_listener(pa_ref, stop_event, interrupted_event):
    ...
    try:
        ...
        if baseline_vals:
            ...
            effective_threshold = max(float(INTERRUPT_RMS_THRESHOLD), bleed_rms * _VOICE_MULTIPLIER)
        else:
            bleed_rms = 0.0
...
def play_pcm_speaking(pcm_bytes, pa, teensy, restore_mouth_idx=0, rate=22050):
    """play_pcm with mouth animation. Cycles open/close bitmaps at 120ms/frame."""
...
def capture_image():
    ...
    for attempt in range(1, 3):  # retry once on transient failure
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                # libcamera spews INFO lines before the real error; find it
                lines = [l for l in err.splitlines() if ' ERROR ' in l or ' WARN ' in l or (l and not l.startswith('['))]
...
def main():
    ...
    try:
        while True:
            ...
            # 150ms drain -- clears beep echo; pre_buf saves any command
            # audio spoken immediately after the wake word.
            ...
            raw = b"".join(_pre_buf) + record_command(mic, ptt_mode=ptt_mode)
