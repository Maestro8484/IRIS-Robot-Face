...
| Teensy 4.1 (eyes+mouth) | **FLASHED S130 live** — `[VER] IRIS-EYES firmware=S130 built=Jun 12 2026`. **S131/S132/S133 firmware REPO-ONLY — pending one operator flash via `scripts\flash_t41.ps1` (flashes the latest = S133).** S131 gates `[SR]` sleep-frame print (RD-031); S132 added `DEBUG_FACE` instrumentation (default off); **S133 = RD-033 FIX: gates the face-acquire `mouthGreet()` out of the tracking path** (new `ENABLE_FACE_GREET` flag, default 0) so the blocking SWSPI surprised-oval redraw no longer stalls eye tracking at acquisition. **⚠️ RD-033 (face-tracking drops ~0.5 s after lock):** root cause confirmed = `mouthGreet()` synchronous SWSPI redraw in the FACE:1 path blocking `loop()`. S133 fix built (REPO-ONLY); **after flashing, eyes should HOLD the lock** (tradeoff: no more "noticed you" mouth greet on acquire). Fallback if still failing: CHANGELOG S133 §FALLBACK. See Active Issues. |
