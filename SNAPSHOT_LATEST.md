...
**Session:** S132 | **Date:** 2026-06-13 | **Branch:** `main` | **Last commit:** S132 (autonomous prep)
...
| Teensy 4.1 (eyes+mouth) | **FLASHED S130 + VERIFIED** — `[VER] IRIS-EYES firmware=S130 built=Jun 12 2026` live. RD-030 #2 emotion-tinted idle + #3 face-acquire greet on the running firmware. **S131 + S132 firmware REPO-ONLY — pending operator flash via `scripts\flash_t41.ps1`.** S131 gates the `[SR]` sleep-frame print (RD-031); S132 adds `DEBUG_FACE` serial instrumentation for RD-033 face-tracking diagnosis (default off — zero cost in production). Flash S132 to observe the tracking drop: `journalctl -u assistant -f \| grep DBG-F`. **⚠️ Face-tracking not sustained (RD-033):** sensor detects + eyes acquire a face but tracking drops after ~0.5 s (operator-confirmed; recurs after code changes). Primary suspect confirmed by code review: `mouthGreet()` BOING animation blocks `loop()` synchronously for ~0–600 ms via SWSPI redraws, starving the tracking window at exactly acquisition time. S132 instruments this to measure block time and confirm. See Active Issues. |
...
- **MEDIUM (RD-033 — autonomous S132 prep done): Person Sensor face-tracking not sustained — acquires then drops after ~0.5 s.** Operator-confirmed (authoritative). Code review (S132) confirms/eliminates candidates: (1) **ed8fa41 `eyeOldX` fix INTACT** — `EyeController.h:518-529` correctly seeds old position on mid-motion restart. (2) **Internal wander timer ELIMINATED** — `applyAutoMove()` returns early when `autoMove=false && !inMotion`; no rogue saccade fires in tracking mode. (3) **PRIMARY: `mouthGreet()` BOING blocking CONFIRMED** — three synchronous SWSPI `fillScreen+draw` calls (T=0ms greet, T≈300ms BOING phase 2, T≈600ms restore via `mouthIdleTick`) block `loop()` for the entire ~0–600 ms tracking window; this matches "acquired then drops at ~0.5 s" exactly, and the symptom recurring "after code changes" aligns with the S130 mouthGreet addition. (4) **Secondary: `box_confidence` flicker** may zero `maxSize` briefly, starving tracking updates. **Next step: operator flashes S132 via `scripts\flash_t41.ps1`, enables `DEBUG_FACE=1` build, watches `journalctl -u assistant -f \| grep DBG-F` at the moment gaze drops.** `[DBG-F] greet block_ms=N` confirms mouthGreet block time; `[DBG-F] mxSz=0 aM=0` after a string of `mxSz>0` confirms confidence flicker. Full bench checklist: `docs/handoff_RD033_person_sensor.md §Autonomous prep`. Fix to test first: gate/skip `mouthGreet()` while tracking is active (i.e. while a face was recently present).
