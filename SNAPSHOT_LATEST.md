...
- **⚠️ TOP PRIORITY (S130, RD-031): log spam / unbounded disk writes — prior space runout crippled the Pi4 (~late May 2026).** Live audit: `[SR] frame` debug print = 52% of recent `assistant` journal lines, `[EYES]` serial echo = 60% (~90% routine serial traffic, continuous all night); journald has no `SystemMaxUse`/`RuntimeMaxUse` cap; `/home/pi/logs` daily exports = 48 MB unbounded (no retention); `/home/pi/.cache/pip` ≈ 150 MB stale. Fix: gate firmware `[SR]` print + bridge serial logging, cap journald (system-path → SD-persist!), add log-export retention, prune pip cache. Full plan + **OPUS** recommendation: `docs/handoff_RD031_logspam.md` / ROADMAP RD-031.
- **MEDIUM (S130): Person Sensor not detected after the S130 flash.** Boot logged `[DBG] No Person Sensor found` (firmware probes I2C once in `setup()`; the post-flash 134-baud bootloader reset can race the sensor power-up). Effect: face tracking + the new face-acquire greet are offline until the Teensy is power-cycled/rebooted and re-probes (`[DBG] Person Sensor detected`). Fix: reboot the Teensy (re-run `scripts\flash_t41.ps1 -SkipBuild`, or power-cycle) — best done while awake to immediately confirm tracking + greet. Pre-existing probe-timing flakiness; not introduced by the S130 firmware (sensor init untouched). Consider a firmware hardening pass (retry/re-probe loop) as a follow-up.
