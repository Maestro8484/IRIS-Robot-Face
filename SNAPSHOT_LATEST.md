...
**Session:** S135 | **Date:** 2026-06-13 | **Branch:** `main` | **Last commit:** S135 (RD-033 root-caused to hardware: T4.1 Person Sensor not detected; firmware exonerated + self-healing re-probe)
...
| Teensy 4.1 (eyes+mouth) | **RD-033 ROOT-CAUSED TO HARDWARE (S135) — the T4.1 Person Sensor is NOT detected on the I2C bus** (`[DBG] No Person Sensor found`, no ACK at 0x62); confirmed across 4 firmware variants + multiple cold power cycles + a **dark sensor LED**. Firmware exonerated (not tracking math, not the mouth greet). **Live firmware = diagnostic `S134R2L`** (rollback + self-healing re-probe + LED-on liveness). **Keeper `S135` BUILT, REPO-ONLY** = full S133 feature set (S131 `[SR]` gate, S132 `DEBUG_FACE` off, S133 greet-gate) + **self-healing re-probe** (boot probe no longer gives up after ~2 s; retries `isPresent()` every 1 s until ACK). **Two Person Sensors exist** — one T4.0/servo, one T4.1/eyes (the dead one); Useful Sensors Person Sensor discontinued. Next: reseat/multimeter → swap T4.0 sensor onto T4.1 bus (dead-vs-bus test) → else Pixy2. See Active Issues + CHANGELOG S135. |
...
- **RD-033 — ROOT-CAUSED TO HARDWARE (S135). The Teensy 4.1 Person Sensor is not detected on the I2C bus** (`[DBG] No Person Sensor found`, no ACK at 0x62). Confirmed across four firmware variants (`DEBUG_FACE=1` diag, `S134R` full rollback past S130, `S134R2` re-probe, `S134R2L` LED-on), multiple full power cycles, and a **dark sensor LED** — firmware is exonerated; this is a **dead or disconnected sensor**, not tracking software. (S133's greet-gate had made tracking *worse*, disproving the mouthGreet hypothesis; this session instrumented-first per protocol and bisected straight to hardware.) Side-finding: a Pi4↔firmware **sleep-state desync** (WebUI SLEEP tab reported "awake" while the firmware was actually asleep, skipping the tracking block) — flagged, separate, not chased. **Durable firmware fix:** a **self-healing re-probe** in `loop()` (boot probe gave up after ~2 s with no retry → now retries `isPresent()` every 1 s until ACK, logging bounded to 30 lines), consolidated as keeper **S135** (= S133 features + re-probe), `pio run` SUCCESS, **REPO-ONLY** (live firmware is still the diagnostic `S134R2L`). **Hardware:** two Useful Sensors Person Sensors mounted side-by-side under the eyes — one T4.0/servo (toggle-gated), one T4.1/eyes (= the dead one); Person Sensor **discontinued**; on-hand **Pixy2** available. **Next:** (1) reseat T4.1 sensor wiring + multimeter 3.3 V at the sensor pins; (2) swap the T4.0's working sensor onto the T4.1 I2C as the definitive dead-vs-bus test (best on the live `S134R2L` build — it has the LED liveness indicator); (3) if dead, replace (Pixy2 on hand / HuskyLens / Grove Vision AI V2 — new-driver project). See CHANGELOG S135 + ROADMAP RD-033.
...
## Last Session Changes (S135 — 2026-06-13, evening)

- **RD-033 root-caused to HARDWARE — the T4.1 Person Sensor is not detected (no I2C ACK at 0x62), confirmed firmware-independent.** Instrument-first bisection after S133 made tracking worse: `DEBUG_FACE=1` (no `[DBG-F]` — IRIS was asleep under a WebUI reporting "awake": a sleep-state desync), full rollback past S130 (`S134R`), self-healing re-probe (`S134R2`), LED-indicator liveness (`S134R2L` — sensor LED stayed **dark**), and a legacy May-23 build (isolated worktree). Four firmware variants + cold power cycles + dark LED ⇒ sensor dead/disconnected.
- **Keeper firmware S135 (REPO-ONLY):** full S133 feature set + a **self-healing re-probe** in `loop()` — boot probe no longer gives up after ~2 s; retries `isPresent()` every 1 s until ACK (bounded logging), then runs the known-good init sequence. Diagnostics reverted (LED off, `DEBUG_FACE` off). `pio run -e eyes` SUCCESS (FLASH 94776). **Live firmware is still the diagnostic `S134R2L`** (re-probe + LED liveness) — ideal for the swap test.
- **Hardware (operator, corrected):** two Useful Sensors Person Sensors side-by-side under the eyes (one T4.0/servo, one T4.1/eyes); Person Sensor **discontinued**. On-hand: the T4.0 sensor (swap candidate) + a **Pixy2**.
- **Forward plan:** reseat/multimeter the T4.1 sensor → swap T4.0's sensor onto the T4.1 bus (dead-vs-bus) → else Pixy2/replacement. See ROADMAP RD-033.
- **Artifacts:** `scripts/flash_t41_legacy.ps1` + isolated worktree `../iris-legacy-S62b` (remove with `git worktree remove ../iris-legacy-S62b` when done).
...
- **RD-033 autonomous prep (REPO-ONLY).** No operator present; bounded prep only. Full code review
  (main.cpp loop/reportFaceState, mouth_tft.cpp/mouthGreet, PersonSensor.cpp read(), EyeController.h
  setTargetPosition read-only). PRIMARY mechanism confirmed: `mouthGreet()` BOING SWSPI blocking
  covers exactly the ~0–600 ms post-acquisition window. SECONDARY: confidence flicker. Internal
  wander timer eliminated. ed8fa41 seeding fix intact. `DEBUG_FACE` gate added (default off) with
  per-read `[DBG-F]` print + `greet block_ms` timing. `FIRMWARE_VERSION` → S132. `pio run -e eyes`
  SUCCESS. Bench checklist in `docs/handoff_RD033_person_sensor.md`. **REPO-ONLY — operator flashes S132.**
