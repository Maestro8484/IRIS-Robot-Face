...
| Teensy 4.1 (eyes+mouth) | **FLASHED S133 + VER-confirmed** — `[VER] IRIS-EYES firmware=S133 built=Jun 13 2026` live (2026-06-13 14:57; assistant active, no errors). Carries S131 (`[SR]` gate, RD-031) + S132 (`DEBUG_FACE`, default off) + **S133 = RD-033 FIX: gates the face-acquire `mouthGreet()` out of the tracking path** (`ENABLE_FACE_GREET` flag, default 0) so the blocking SWSPI surprised-oval redraw no longer stalls eye tracking at acquisition. **RD-033 fix S133 FLASHED but tracking OBSERVED WORSE (2026-06-13) — `mouthGreet()` hypothesis NOT confirmed; RD-033 STILL OPEN. Next session: instrument with `DEBUG_FACE=1` first, consider reverting `4499c3d`.** See Active Issues + CHANGELOG S133 §FALLBACK. |
...
- **RD-033 — STILL OPEN. S133 fix FLASHED but made tracking WORSE (operator-observed 2026-06-13). The `mouthGreet()`-blocking hypothesis is NOT confirmed — gating the greet (commit `4499c3d`) did not help and degraded behavior, so it is likely wrong or incomplete.** Person Sensor face-tracking not sustained. **Next session must INSTRUMENT FIRST (DEBUG_FACE=1) — no more blind code-review fixes — and should consider reverting `4499c3d` to re-establish a baseline.** Operator-confirmed (authoritative). Code review (S132) confirms/eliminates candidates: (1) **ed8fa41 `eyeOldX` fix INTACT** — `EyeController.h:518-529` correctly seeds old position on mid-motion restart. (2) **Internal wander timer ELIMINATED** — `applyAutoMove()` returns early when `autoMove=false && !inMotion`; no rogue saccade fires in tracking mode. (3) **PRIMARY: `mouthGreet()` BOING blocking CONFIRMED** — three synchronous SWSPI `fillScreen+draw` calls (T=0ms greet, T≈300ms BOING phase 2, T≈600ms restore via `mouthIdleTick`) block `loop()` for the entire ~0–600 ms tracking window; this matches "acquired then drops at ~0.5 s" exactly, and the symptom recurring "after code changes" aligns with the S130 mouthGreet addition. (4) **Secondary: `box_confidence` flicker** may zero `maxSize` briefly, starving tracking updates. **FIX (S133): `mouthGreet()` gated out of the FACE:1 path** via new `ENABLE_FACE_GREET` compile flag (default 0) in `src/main.cpp` — acquisition no longer triggers the blocking SWSPI redraw, so tracking should hold. Tradeoff: no "noticed you" mouth greet on face-acquire (re-enable + rework non-blocking later). **RESULT: S133 FLASHED → tracking WORSE (operator-observed). Hypothesis unconfirmed.** **Next session: (a) get the operator to characterize "worse" exactly; (b) build `DEBUG_FACE=1`, flash, read `grep DBG-F` at the failure moment BEFORE changing anything; (c) strongly consider `git checkout 4499c3d~1 -- src/main.cpp src/config.h` to revert to baseline and A/B compare. Then work the suspects:** see CHANGELOG S133 §FALLBACK — rebuild `DEBUG_FACE=1`, watch `grep DBG-F`, then secondary suspects (confidence flicker at main.cpp:440, mouth idle-engine blocking during tracking, premature autoMove). Bench checklist: `docs/handoff_RD033_person_sensor.md`.
