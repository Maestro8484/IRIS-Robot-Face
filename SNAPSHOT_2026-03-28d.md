...
**Last commit:** `bcef0fd` Fix eyelid droop, startup volume/OWW threshold, add snapshot
...
### Uncommitted working-copy changes (need commit before next flash)
| File | Change |
|---|---|
| `src/main.cpp` | Added [SLEEP] debug serial: prints when face detected while eyesSleeping=true (throttled 3s) |
| `src/mouth.h` | `mouthInit()` intensity 0x01 → 0x05, matches `mouthRestoreIntensity` |
| `CLAUDE.md` | Added SessionStart hook note, `/flash`/`/flash-remote` refs, key files section, end-of-session rule, UDP serial ownership rule |

**Untracked files not yet committed:** AUDIT_2026-03-27.md, SNAPSHOT_2026-03-28.md, SNAPSHOT_2026-03-28b.md, SNAPSHOT_2026-03-28c.md, SNAPSHOT_2026-03-28d.md, replacements.txt
...
**Note:** ElevenLabs API key is in assistant.py plaintext — do NOT include in snapshots or public commits.
...
### HIGH
- **Person sensor brief eye-open during sleep** — [SLEEP] debug serial now in firmware (main.cpp, guard block after reportFaceState). Not yet flashed/confirmed. Next step: flash firmware, capture serial log when face detected during sleep, confirm "[SLEEP] face detected - eyesSleeping=true, guard holding" appears and no eye-open occurs.
- **"10-min re-sleep" root cause unconfirmed** — UDP fix removes serial conflict theory. If recurs, capture [SLEEP] serial log.
- **Uncommitted working-copy changes** — main.cpp ([SLEEP] debug), mouth.h (intensity 0x01→0x05), CLAUDE.md (doc updates). Need `git add` + `git commit` before next flash to avoid losing them.

### MEDIUM
- **APA102 indigo breathe during sleep** — On EYES:SLEEP: set LEDs to indigo breathe (floor=1, peak=6, period=8s, sine, RGB=(20,0,40)). Add to CMD listener + wakeword sleep handler in assistant.py.
- **config.eye pupil sync** — nordicBlue/hazel/bigBlue config.eye have old values. Update before next genall.py run: nordicBlue min=0.21/max=0.47, hazel/bigBlue min=0.26/max=0.60.
...
- **Go public on GitHub** — Strip ElevenLabs API key from iris_web.py and assistant.py first.
- **Untracked files to commit** — AUDIT_2026-03-27.md, all SNAPSHOT_2026-03-28*.md, replacements.txt. Commit as a batch: `git add AUDIT_2026-03-27.md SNAPSHOT_2026-03-28*.md replacements.txt && git commit -m "Add 2026-03-28 audits and snapshots"`

---

## 12. CHANGES THIS SESSION

Session 2026-03-28d was a snapshot-only session (no new feature work).

**Carried-forward uncommitted changes from previous sessions (still need commit + flash):**

- `src/main.cpp` (lines ~318–325): Added [SLEEP] guard debug — after `reportFaceState()`, if `eyesSleeping && maxSize > 0`, prints `[SLEEP] face detected - eyesSleeping=true, guard holding` throttled to every 3s via `lastSleepDbg`. Verifies the guard is holding without visually opening eyes.
- `src/mouth.h` (`mouthInit()`, line ~181): Startup intensity 0x01 → 0x05. Ensures mouth LEDs start at ~33% matching `mouthRestoreIntensity`, not near-off.
- `CLAUDE.md`: Added SessionStart hook documentation, replaced raw flash instructions with `/flash`/`/flash-remote` references, added UDP serial ownership rule, added key files section, added end-of-session `/snapshot` rule.

---

## 13. WHAT HAPPENED IN RECENT SESSIONS
...
- CLAUDE.md improved: hook docs, flash commands, key files, serial ownership rule
...
**Commit uncommitted working-copy changes:**
```bash
git add CLAUDE.md src/main.cpp src/mouth.h && git commit -m "Add [SLEEP] debug, fix mouthInit intensity, update CLAUDE.md"
git add AUDIT_2026-03-27.md SNAPSHOT_2026-03-28.md SNAPSHOT_2026-03-28b.md SNAPSHOT_2026-03-28c.md SNAPSHOT_2026-03-28d.md replacements.txt && git commit -m "Add 2026-03-28 audits, snapshots, replacements"
```
...
- Eye controller: `src/eyes/EyeController.h` — do NOT break setTargetPosition seed fix
