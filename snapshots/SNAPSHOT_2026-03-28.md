...
**SSH MCP tools:** `ssh-pi4` (192.168.1.200), `ssh-gandalf` (192.168.1.3)
...
## 3. CLAUDE CODE INFRASTRUCTURE (NEW — 2026-03-28)
...
- `teensy-loader-cli` installed at `/usr/bin/teensy_loader_cli`
...
- `/etc/udev/rules.d/49-teensy.rules` — udev rules for HalfKay (0x0478) + serial (0x0483)
- All four files persisted to `/media/root-ro`, md5 verified

**Remote flash workflow:** `pio run` on desktop → `scp firmware.hex to Pi4 /tmp/` → stop assistant → press PROG → `sudo teensy_loader_cli --mcu=TEENSY40 -w -v /tmp/iris_firmware.hex` → start assistant. See `/flash-remote` for full steps.
...
IRIS-Robot-Face/
  ...
  .claude/
    commands/                   -- slash commands: flash, flash-remote, deploy, snapshot
...
SD is read-only. All SSH writes go to RAM, wiped on reboot.

**Persist any file (or use `/deploy` command):**
```bash
...
|---|---|---|
| HIGH | **Person sensor brief eye-open during sleep** | [SLEEP] debug serial added to firmware. Guard logic confirmed correct. Need to capture serial during sleep to determine if Pi4 is sending a wake command on FACE:1, or if it's a display artifact. Next: observe serial output when face detected during sleep. |
| HIGH | **"10-min re-sleep" root cause unconfirmed** | UDP fix removes serial conflict theory. If recurs, capture [SLEEP] serial log on Teensy. |
| MED | **APA102 indigo breathe during sleep** | Not implemented. On EYES:SLEEP: set LEDs to indigo breathe (floor=1, peak=6, period=8s, sine, RGB=(20,0,40)). Add to CMD listener EYES:SLEEP path and wakeword sleep handler in assistant.py. |
| MED | **config.eye pupil sync** | nordicBlue/hazel/bigBlue config.eye still have pre-edit values. Update to match .h: nordicBlue min=0.21/max=0.47, hazel/bigBlue min=0.26/max=0.60. Do before next genall.py run or it will overwrite the manual edits. |
| MED | **EMOTION:CONFUSED missing from assistant.py** | Teensy handles it. assistant.py never sends it. Add to VALID_EMOTIONS, MOUTH_MAP, emit_emotion(). |
| MED | **MOUTH:n not sent alongside EMOTION:n** | emit_emotion() sends EMOTION:X only. Should send MOUTH:MOUTH_MAP[X] alongside. Review mouth pattern assignments. |
...
- [x] Citadel L2→L3/L4 infrastructure built: /flash-remote, /deploy, /snapshot commands
- [x] SessionStart hook: auto-loads latest snapshot into every Claude Code session
- [x] PostToolUse hook: pio run compile check after every src/ edit
- [x] teensy-loader-cli installed on Pi4, sudoers configured, udev rules installed
- [x] All Pi4 config files persisted to SD (sudoers, udev), md5 verified
- [x] CLAUDE.md updated: session start pointer, hook references, serial UDP rule
- [x] Remote flash workflow documented and tested (dry run confirmed no permission errors)

---

## 11. WHAT HAPPENED IN RECENT SESSIONS

### 2026-03-27 (session 1)
- Diagnosed + fixed EYES:SLEEP Teensy freeze: updateChangedAreasOnly(true) caused SPI DMA lockup. Fixed by disabling on EYES:SLEEP, restoring on EYES:WAKE (commit `173b485`)
- Diagnosed + fixed vision capture: --immediate flag, retry, proper stderr parsing
- Diagnosed + fixed face tracking: setTargetPosition did not advance eyeOldX/Y on motion restart. Fixed with smoothstep seed (commit `ed8fa41`)

### 2026-03-27 (session 2)
- Removed [TRK] debug serial prints from main.cpp (commit `0bb76e6`)
- mouthInit brightness 0x01 → 0x05 (matches post-wake restore level)
- [SLEEP] debug serial added to person sensor block
- iris_sleep.py / iris_wake.py switched to UDP path
- CMD listener syncs _eyes_sleeping + flag file for all paths
- squint 0.5 → 1.0 on nordicBlue.h, hazel.h, bigBlue.h
- assistant.py set_volume(110) at startup
- iris_config.json: OWW_THRESHOLD=0.85, VOL_MAX=110, persisted

### 2026-03-28 (this session — Claude Desktop / Chat)
- Built full Citadel L3/L4 infrastructure for Claude Code handoff continuity
- SessionStart + PostToolUse hooks wired
- /flash-remote, /deploy, /snapshot slash commands created
- teensy-loader-cli installed and configured on Pi4 for remote flashing
- Reliability-first policy applied: baud-134 bootloader trick removed, PROG button required

---

## 12. QUICK REFERENCE

**Restart assistant (Pi4):**
```bash
