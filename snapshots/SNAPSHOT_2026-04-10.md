...
**Session:** 8
**Branch:** `main`
**Last commit:** `eca1627` (feat: Person Sensor tracking — confirmed working, stock chrismiller code, sensor right-side-up)
...
| Teensy 4.0 | USB → Desktop PC | N/A | Dual GC9A01A 1.28" round TFT eyes + ILI9341 2.8" TFT mouth |
...
**Firmware:** Flashed 2026-04-10 (S8). CONFIRMED GOOD WORKING STATE.
...
**ILI9341 TFT mouth:** WORKING. Arduino_GFX + Arduino_SWSPI. NEUTRAL cyan upward-curved on boot.
...
### ILI9341 TFT Mouth (bit-bang SPI)
| Pin | Signal |
|---|---|
| 4 | RST |
| 5 | MOSI |
...
## 12. CHANGES THIS SESSION (S8 — 2026-04-10)

- **Person Sensor physically remounted RIGHT-SIDE-UP** — was causing all tracking issues
- **Tracking code reverted to stock chrismiller** — all session tweaks (conf threshold, poll rate, is_facing removal, coordinate flips, debug prints, lastQualifiedFaceMs) removed
- `src/sensors/PersonSensor.h` — SAMPLE_TIME_MS restored to 70
- `src/main.cpp` — tracking loop fully stock, no debug output
- `src/TeensyEyes.ino.cpp` — deleted (generated artifact, was gitignored anyway)
- **CONFIRMED WORKING** after flash: eye tracks face, lateral movement follows correctly
