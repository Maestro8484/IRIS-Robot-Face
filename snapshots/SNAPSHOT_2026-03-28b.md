...
| Synology NAS | 192.168.1.102 | Master / Gateway!7007 | Backups, media. SSH port 2233. CIFS share: //192.168.1.102/BACKUPS |

**Claude Desktop MCP filesystem scope:** `C:\Users\SuperMaster`
**SSH MCP tools:** `ssh-pi4` (192.168.1.200), `ssh-gandalf` (192.168.1.3), `ssh` (general, used for NAS port 2233)
...
### Remote Flash Prerequisites (all done, persisted to SD)
...
- All persisted to `/media/root-ro`, md5 verified

---

## 4. NAS BACKUP

**Method:** CIFS mount + tar. Matches the manual method used 2026-03-25.
**Script:** `/home/pi/iris_backup.sh` (persisted to SD)
**Mount:** `//192.168.1.102/BACKUPS` → `/mnt/nas` via `mount -t cifs`
**Runs:** Cron Sunday 3AM — `0 3 * * 0 sudo /bin/bash /home/pi/iris_backup.sh`
**Log:** `/home/pi/logs/iris_backup.log`
**Keeps:** Last 4 backups (pi4-iris-*.tar.gz), auto-prunes older
**NAS location:** `/volume1/BACKUPS/pi4-iris-<date>.tar.gz`
**Current backups on NAS:**
- `pi4-jarvis-2026-03-25.tar.gz` (1.3GB) — manual, pre-automation
...
- [x] SessionStart hook: auto-loads latest snapshot each Claude Code session
- [x] PostToolUse hook: pio run compile check after every src/ edit
- [x] teensy-loader-cli + sudoers + udev rules on Pi4, all persisted to SD
- [x] NAS backup automated: CIFS tar, weekly cron Sunday 3AM, keeps 4 backups
- [x] iris_backup.sh + sudoers/iris_backup persisted to SD, md5 verified
- [x] First automated backup confirmed: pi4-iris-2026-03-28.tar.gz (1.1GB) on NAS
...
- Remote flash via Pi4 SSH fully configured (teensy-loader-cli, sudoers, udev)
- NAS backup automated via CIFS (matches prior manual method)
...
| Synology NAS | 192.168.1.102 | Master / Gateway!7007 | SSH port 2233, CIFS shares |
