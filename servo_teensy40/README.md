...
`teensy40_base_mount/teensy40_base_mount.ino` — orchestration sketch (setup/loop). Subsystems are modular: `paj7620.*` (gesture driver), `person_sensor.*` (face decode + tracking gate), `pan_servo.*` (ServoEasing wrapper), `diag.h` (serial diagnostic macros). Build with PlatformIO env `teensy40`, user performs upload.
