...
| Teensy 4.1 | **FLASHED S133 + VER-confirmed (2026-06-13 14:57)** — `[VER] IRIS-EYES firmware=S133 built=Jun 13 2026` live; assistant active, no errors. Carries S131 (`[SR]` gate, RD-031) + S132 (`DEBUG_FACE`, default off) + **S133 = RD-033 fix** (gates the blocking face-acquire `mouthGreet()` out of the tracking path via `ENABLE_FACE_GREET=0`, fix commit `4499c3d`). **RD-033 fix LIVE — tracking-hold behavioral check pending operator observation.** If a face still drops: CHANGELOG S133 §FALLBACK. |
...
- **RD-033 FIX — fix commit `4499c3d` (S133).** Root cause (confirmed S132 code review, root-caused S133):
  ...
  tracking loop at acquisition. Fix: gated the greet behind `ENABLE_FACE_GREET` (default 0) in
  `src/main.cpp`; the FACE:1 path no longer blocks. Built clean, REPO-ONLY.
  **FALLBACK if tracking STILL drops after flashing S133 (precise, low-context restart):** start a session,
  `git show 4499c3d` for the exact change, then follow **CHANGELOG.md → S133 → §FALLBACK** (rebuild
  `DEBUG_FACE=1`, watch `grep DBG-F`, then secondary suspects: confidence flicker at `src/main.cpp:440`,
  mouth idle-engine SWSPI blocking during tracking, premature autoMove). Bench checklist:
  `docs/handoff_RD033_person_sensor.md`. Reverting the fix: `git checkout 4499c3d~1 -- src/main.cpp src/config.h`.
- **DONE (S131): RD-031 log-spam elimination (Pi4 DEPLOYED+VERIFIED; firmware REPO-ONLY) + RD-032 WebUI resource monitor (DEPLOYED+VERIFIED).** Bridge `teensy_bridge.py` default-off `IRIS_DEBUG_SERIAL` gate kills routine `[SR]`/`MOUTH` echoes (~90% of spam; verified 0 `>> MOUTH`/25 s, md5 RAM=SD `10f189ae`); journald capped 50M/50M via `journald.conf.d/iris.conf` drop-in (was *expanding* to 500M/1yr), main conf reverted, `cat-config` confirms, md5 RAM=SD; firmware `sleep_renderer.h` `[SR]` print gated `#ifdef DEBUG_SR`, `FIRMWARE_VERSION`→S131, builds clean (REPO-ONLY); daily log export was already 100M-capped (stale finding); pip cache 169M→0; audit found no other unbounded writer. RD-032: `/api/sysstat` (computed on request, never logged) + Resource Monitor card + 10 s poll/sparkline, md5 RAM=SD on all 3 WebUI files. See CHANGELOG S131. (Note: Pi rebooted ~08:45 today, so res_trend.csv had only ~9 min of data this session.)
