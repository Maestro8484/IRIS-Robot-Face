...
**Sleep LEDs:** APA102 switches to dim indigo breathe (floor=1, peak=8, 8s period) on EYES:SLEEP. Restores idle on EYES:WAKE.
...
**Voice pipeline (assistant.py):** Operational. ElevenLabs disabled via iris_config.json. Modular (hardware/, core/, services/, state/).
...
**ElevenLabs:** Disabled — `ELEVENLABS_ENABLED=False` in `/home/pi/iris_config.json` (persisted to SD). Default in config.py is still `True`; override wins.
...
IRIS-Robot-Face/
  ...
  /home/pi/ (Pi4, all persisted to SD):
    assistant.py                -- voice pipeline, TeensyBridge, CMD listener (leds threaded in)
    ...
    hardware/led.py             -- APA102 driver; show_sleep() (dim indigo breathe)
    iris_config.json            -- web UI runtime overrides (persisted to SD)
    iris_sleep.py               -- 9PM cron (UDP only)
    iris_wake.py                -- 7:30AM cron (UDP only)
    iris_web.py                 -- web UI Flask (port 5000); send_teensy() uses UDP
...
sleep        → show_sleep()      indigo breathe, floor=1, peak=8, 8s
...
## 11. ASSISTANT.PY / CONFIG KEY VALUES (Pi4)

```python
# core/config.py defaults (may be overridden by iris_config.json)
GANDALF             = "192.168.1.3"
ELEVENLABS_VOICE_ID = "90eMKEeSf5nhJZMJeeVZ"
ELEVENLABS_MODEL    = "eleven_turbo_v2_5"
ELEVENLABS_ENABLED  = True          # ← default; iris_config.json overrides to False
...
TEENSY_BAUD         = 115200
VOL_MAX             = 127           # ← default; override in iris_config.json if needed
```

**iris_config.json** (`/home/pi/iris_config.json`, persisted to SD):
```json
{"ELEVENLABS_ENABLED": false}
```

**_OVERRIDABLE keys** (the only keys iris_config.json can set — unknown keys are logged and ignored):
`RECORD_SECONDS, SILENCE_SECS, SILENCE_RMS, KIDS_RECORD_SECONDS, KIDS_SILENCE_SECS,
KIDS_SILENCE_RMS, OWW_THRESHOLD, FOLLOWUP_TIMEOUT, KIDS_FOLLOWUP_TIMEOUT,
FOLLOWUP_MAX_TURNS, CONTEXT_TIMEOUT_SECS, NUM_PREDICT, ELEVENLABS_VOICE_ID,
ELEVENLABS_MODEL, ELEVENLABS_ENABLED, VOL_MAX, OLLAMA_MODEL_ADULT, OLLAMA_MODEL_KIDS`

**Note:** LED_* keys are NOT overridable via iris_config.json (removed from loader this session).
**Note:** ElevenLabs API key is in config.py plaintext — do NOT include in snapshots or commits.
...
sudo cp /home/pi/<file> /media/root-ro/home/pi/<file>   # use sudo cp — root owns files
sudo mount -o remount,ro /media/root-ro
md5sum /home/pi/<file> /media/root-ro/home/pi/<file>
```

**Note:** `cp` without sudo fails even after remount — files are root-owned on /media/root-ro.
...
# 3. Stop assistant (use MCP ssh-pi4 with password auth)
...
# 5. Flash
sudo teensy_loader_cli --mcu=TEENSY40 -w -v /tmp/iris_firmware.hex
...
# Persist file to SD (note: sudo cp required — files are root-owned)
sudo mount -o remount,rw /media/root-ro
